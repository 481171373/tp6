...
class DBRSAUtil{
    ...
    public static function sign($data, $privateKeyFilePath) {
        ...
        $res = openssl_get_privatekey($privateKeyFilePath);
    ...
    public static function verify($data, $sign, $rsaPublicKeyFilePath) {
        ...
        //转换为openssl格式密钥
        $res = openssl_get_publickey($rsaPublicKeyFilePath);
    ...
    public static function rsaEncrypt($data, $pubKeyFilePath){
        ...
        //转换为openssl格式密钥
        $res = openssl_get_publickey($pubKeyFilePath);
        ...
        while($data){
            ...
            openssl_public_encrypt($input,$encrypted,$pubKeyFilePath);
            $output.= $encrypted;
        }
    ...
    public static function rsaDecrypt($data, $privateKeyFilePath){
       ...
       // $priKey = file_get_contents($privateKeyFilePath);
        ...
        $maxlength = DBRSAUtil::getMaxDecryptBlockSize($res);
        $output='';
        while($data){
            $input = substr($data,0,$maxlength);
            $data = substr($data,$maxlength);
